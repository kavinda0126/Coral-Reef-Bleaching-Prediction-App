...
Models are loaded from the local models/ folder committed to the HF Space repo.
No Google Drive download needed — HF Spaces serves files from the repo directly.
...
# ── Model loading ──────────────────────────────────────────────────────────────
# HF Spaces: models are in the models/ folder in the repo root.
# No download needed — files are committed alongside app.py.
...
# Resolve models/ folder relative to this file's location
...
for name, prefix in MODEL_PREFIXES.items():
    # HF Space stores models in subfolders: models/lr/, models/rf/, etc.
    # Fall back to flat models/ if subfolder not found (Colab compatibility)
    sub = os.path.join(MODELS_DIR, prefix)
    base = sub if os.path.isdir(sub) else MODELS_DIR

    mpath = os.path.join(base, f"{prefix}_model.pkl")
    fpath = os.path.join(base, f"{prefix}_features.pkl")
    spath = os.path.join(base, f"{prefix}_scaler.pkl")
    jpath = os.path.join(base, f"{prefix}_metadata.json")
    ...
    try:
        ...
        # LR has no scaler file (Pipeline handles scaling internally)
        # SVM has svm_scaler.pkl
        ...
        print(f"  Loaded: {name:<25} pipeline={is_pipe}  "
              f"needs_scaling={needs_sc}  size={size_kb}KB")
...
print(f"\nLoaded {len(MODELS)}/{len(MODEL_PREFIXES)} models: {list(MODELS.keys())}")
...
# Sample reef sites — temperatures in KELVIN, DHW within training range 0-2.97
SAMPLE_SITES = {
...
def predict_with_model(name, model, X_raw_df):
    ...
    Routes correctly for every model type:
      ...
      LR  (Pipeline) — X is passed as a raw DataFrame directly.
      ...
      SVM            — SCALERS[name] is svm_scaler, needs_scaling=True.
    ...
    # Align columns to what this model was trained on
    cols = [f for f in feats if f in X_raw_df.columns]
    X    = X_raw_df[cols].copy()

    if isinstance(model, _SkPipeline):
        # LR: Pipeline handles scaler + poly + LR internally
        # Pass raw DataFrame — do NOT convert to numpy
        pass

    elif needs_sc and scaler is not None:
        # SVM: apply the saved scaler
        n_exp = scaler.n_features_in_
        if X.shape[1] != n_exp:
            # Pad any missing features with 0
...
def run_prediction(ClimSST, Temperature_Mean, Temperature_Minimum, Temperature_Maximum,
                   ...
                   Distance, Depth, Latitude_Degrees, Longitude_Degrees, Date_Year):
    if not MODELS:
        return "No models loaded. Check the models/ folder in the Space repo.", None

    inp = dict(zip(ALL_FEAT_COLS, [
        float(ClimSST), float(Temperature_Mean), float(Temperature_Minimum),
        float(Temperature_Maximum), float(SSTA), float(SSTA_DHW), float(TSA),
        float(TSA_DHW), float(TSA_DHW_Frequency), float(Windspeed),
        float(SSTA_Frequency), float(SSTA_Frequency_Standard_Deviation),
        float(Turbidity_ct), float(Turbidity), float(Cyclone_Frequency),
        float(Distance), float(Depth), float(Latitude_Degrees),
        float(Longitude_Degrees), float(Date_Year),
    ]))
    ...
    for n, p, pred in zip(names_out, probs_out, preds_out):
        bar = chr(9608) * int(p * 20) + chr(9617) * (20 - int(p * 20))
    ...
    wedge_val = max(1.0, min(avg, 99.0))  # avoid 0/100 pie edge cases
    ax2.pie([wedge_val, 100 - wedge_val], colors=[gc, "#2d3748"],
...
# ── Tab 2: Sample site loader ──────────────────────────────────────────────────
def load_and_predict_site(site_name):
...
def run_batch():
    ...
    for site, vals in SAMPLE_SITES.items():
        ...
        for name, model in MODELS.items():
            try:
                ...
                print(f"Batch error {name}/{site}: {e}")
                prob = 0.5
    ...
    ax.set_xticklabels([s[:28] for s in site_names], rotation=45,
...
def run_dataset_validation():
    """
    Downloads the BCO-DMO dataset, rebuilds the exact test split used in training
    (random_state=42, stratify=y, test_size=0.2), picks 12 balanced real records,
    runs all 4 models, and shows predictions vs actual ground truth.
    """
    if not MODELS:
        return "No models loaded.", None
    ...
    LOCAL_CSV   = os.path.join(BASE_DIR, "global_bleaching_environmental.csv")
    PRIMARY_URL = ("https://datadocs.bco-dmo.org/dataset/773466/file/"
                   "B11vA82u7y2Owp/global_bleaching_environmental.csv")

    if not os.path.exists(LOCAL_CSV):
        try:
            print("Downloading BCO-DMO dataset for validation...")
    ...
    if BLEACH_COL is None:
        return "Could not find bleaching column in dataset.", None
    ...
    feats = [c for c in ALL_FEAT_COLS if c in df.columns and
    ...
    # Pick 6 bleaching + 6 no-bleaching from held-out test set
    ...
    txt += f"  Source: BCO-DMO Global Coral Bleaching Database\n"
    txt += f"  Split : held-out test set (random_state=42, test_size=0.2)\n"
    txt += f"  Sample: {(y_val==1).sum()} bleaching + {(y_val==0).sum()} no-bleaching\n"
    txt += "=" * 72 + "\n"
    ...
    txt += "=" * 72 + "\n"
    txt += "  Legend: OK = correct | XX = wrong | Probability = P(bleaching)"
    ...
    ax1.set_title(f"Model Accuracy — {len(results)} Real BCO-DMO Records",
                  color="white", fontweight="bold")
    ax1.tick_params(colors="white")
    ax1.spines[:].set_visible(False)
    for bar, acc, n in zip(bars, accs, model_names):
        ax1.text(min(acc + 1.5, 114),
                 bar.get_y() + bar.get_height() / 2,
    ...
    ax2.set_title("Correct (v) / Wrong (X)   B=Bleaching  N=No-bleaching",
...
# ── Slider config — Kelvin, DHW max 3.0 (training range 0.00-2.97) ─────────────
SLIDER_CFG = [
...
# ── Gradio UI ──────────────────────────────────────────────────────────────────
loaded_str = ", ".join(MODELS.keys()) if MODELS else "None — check models/ folder in repo"
...
""")
    ...
    with gr.Tabs():
        ...
        with gr.Tab("Sample Reef Sites"):
            ...
            load_btn.click(fn=load_and_predict_site, inputs=[site_dd],
                           outputs=[site_txt, site_chart])

            rows = ["| # | Site | ClimSST (K) | Temp Mean (K) | SSTA | TSA_DHW | Year |",
                    "|---|---|---|---|---|---|---|"]
        ...
        with gr.Tab("Dataset Validation"):
            gr.Markdown("""
### Real Records from the BCO-DMO Dataset — Predictions vs Actual Ground Truth

Downloads the actual BCO-DMO Global Coral Bleaching Database, rebuilds the
**exact same test split** used during training (`random_state=42`, `stratify=y`,
`test_size=0.2`), and selects **12 balanced real records**
(6 bleaching + 6 no-bleaching) from the held-out test set.

Each row shows:
- **Actual** — the real bleaching status from the dataset
- **LR / RF / XGB / SVM** — each model prediction (`OK` = correct, `XX` = wrong) with probability
- **Agreement** — how many models got it right

> Requires internet access to download the dataset on first run (~50 MB). Cached locally after that.
""")
            val_btn   = gr.Button("Run Dataset Validation", variant="primary", size="lg")
            val_txt   = gr.Textbox(label="Scoreboard — Predictions vs Actual Labels",
                                   lines=24, elem_classes=["result-box"])
            val_chart = gr.Plot(label="Accuracy Bars + Correctness Heatmap")
        ...
        with gr.Tab("Model Info"):
            ...
            info_rows = ["| Model | Type | Features | Test Accuracy | F1 | ROC-AUC |",
                         "|---|---|---|---|---|---|"]
            for name, meta in METADATA.items():
                mtype = ("Pipeline (scaler+poly+LR inside)"
                         if isinstance(MODELS[name], _SkPipeline) else "Standard")
                info_rows.append(
...
| Model | Type | Scaler |
|---|---|---|
| Logistic Regression | sklearn Pipeline | None — built into Pipeline |
...
**Bleaching risk thresholds (training data):**
- `TSA_DHW > 2.5` — high risk  (training range: 0.00-2.97)
